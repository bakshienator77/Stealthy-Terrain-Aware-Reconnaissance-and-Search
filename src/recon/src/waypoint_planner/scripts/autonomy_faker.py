...
import math
from aidtr_msgs.msg import AutonomyActionGoal, AutonomyGoal, AutonomyAction
import threading
...
class AutonomyServer:
    ...
    def traversability_costmap_load_and_publish(self):
        ...
        working_dir = "/home/" + str(os.environ['USER']) + "/src/aidtr-autonomy/"
        img = np.flipud(rgb2gray(image.imread(
            working_dir + "autonomy_assets/aidtr_locations/" + rospy.get_param("~location") + "/map/" + rospy.get_param(
                "~map_type") + "_map.png")))
